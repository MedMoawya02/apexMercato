...
class Transfert
{
    ...
    //public function getTransferts paginate
     public function allTransfertsPaginated($limits, $offset)
    {
        $this->db = new Database("localhost", "apexmercato", "root", "");
        $conn = $this->db->getConnection();
        $sql = "SELECT 
        ...
        LEFT JOIN coach 
         ...
         ";
        $stmt = $conn->prepare($sql);
        $stmt->bindValue(':limit', (int) $limits, PDO::PARAM_INT);
    ...
    //
    public function countTransferts()
    {
        $this->db = new Database("localhost", "apexmercato", "root", "");
        $conn = $this->db->getConnection();
